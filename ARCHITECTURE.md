...
The Velopack updater (Features/Updates) follows a user-selectable channel: `AppSettings.UpdateChannel` (empty = follow the channel this build was installed from, derived from the running version's pre-release suffix). `UpdateService` pins `UpdateManager` to it via `ExplicitChannel` + `AllowVersionDowngrade` (a dev→stable switch is a downgrade, applied through the same explicit "Restart & update" path). Switching is surfaced in Settings → Updates on installed builds only.
...
- **Action buttons.** A message's ntfy `actions` (up to three) render as a button row, parsed and persisted exactly like attachments — a single forward-compatible `actions` JSON column on `messages` (via `EnsureColumn`), carried on `NtfyMessage.Actions` / `HistoryMessage.Actions`. All execution funnels through `MessageActionInvoker` (singleton) so the safety rules live in one place: `view` opens the URL through `SafeUrl` (scheme allow-list), `copy` writes the value to the clipboard, and `http` — a publisher-controlled request — shows a confirmation dialog with the method + URL and only fires on approval, **never attaching the server bearer token** (the URL is arbitrary; injecting the token would leak it). `broadcast` is Android-only and anything unsupported renders as a disabled button with an explanatory tooltip. The same actions also appear on the **Windows toast** (`ToastNotifier` emits an `<actions>` block, schema order visual → actions → audio). Every button activates through our own `ntfy-desktop://` scheme rather than launching a URL directly — a toast-activated app receives the foreground grant, so when *it* opens the browser the page comes to the foreground, whereas a raw URL handed to the shell only flashes in the taskbar. A `view` button carries its URL (`ntfy-desktop://view?url=<url>`, self-contained, no history needed): the freshly-launched second instance opens it and exits (it holds the foreground grant), and on cold start the first instance opens it from `DispatchActivation`. `http`/`copy` buttons carry `ntfy-desktop://action?msg=<id>&i=<index>` and need the running instance: `App.DispatchActivation` resolves the message via `HistoryRepository.GetByMessageId`, takes `Actions[index]`, and runs it through the same `MessageActionInvoker` (so `http` is still confirmed — the app is brought forward for that dialog — while `copy` stays silent). All toast `arguments` go through `SafeUrl`/scheme validation; `broadcast`/unsupported actions are omitted from the toast.
...
- **Markdown bodies (subset).** When ntfy flags a message `text/markdown` (the `content_type` field, set by the `Markdown:` publish header), the feed renders a deliberately small subset — **bold, italic, inline code, fenced code blocks, links, and line breaks** — via a hand-written renderer (`Features/Feed/Markdown/MarkdownRenderer.cs`); no CommonMark library, so the dependency footprint stays nil. Headings, lists, tables and blockquotes are out of scope and pass through as plain text. The renderer is a recursive-descent inline parser (anything that isn't a *valid* span is emitted as literal text, so a stray `*` or unmatched `[` never eats content) that produces WPF block elements; `_` emphasis honours the CommonMark intraword rule so `snake_case`/`_filename_` tokens inside a markdown body aren't italicised, and a link only becomes a clickable `Hyperlink` when its URL passes `SafeUrl` (others render as inert label text). The row body is a `StackPanel` filled by the `MarkdownBody` attached properties (`Text` + `IsMarkdown`), re-rendering when a virtualized row recycles onto a new message; **rendering is gated on the `content_type` flag and never auto-detected** in a plain body (auto-detecting would mangle plain text like `_underscored_filenames_`). `content_type` rides through the pipeline like the other message fields (`NtfyMessage` → a `content_type` column on `messages` via `EnsureColumn` → `HistoryMessage.IsMarkdown`). Toasts still show the raw text — the Windows toast template can't render markdown — so this is a feed-only enrichment.
